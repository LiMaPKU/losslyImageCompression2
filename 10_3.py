...
class InpaintNet(nn.Module):
    def __init__(self):
        ...
        self.conv3_0 = nn.Conv2d(128, 128, 3, padding=1)
        ...
        self.conv_channels_down = nn.Conv2d(128, 1, 1)

    def forward(self, x):
        x = F.leaky_relu(self.conv_channels_up(x))

        xA = x
        x = self.bn3_0(F.leaky_relu(self.conv3_0(x)))
        x = self.bn3_1(F.leaky_relu(self.conv3_1(x)))
        x = self.bn3_2(F.leaky_relu(self.conv3_2(x)))
        x = self.bn3_3(F.leaky_relu(self.conv3_3(x)))
        x = x + xA
...
if(sys.argv[2]=='0'): # 设置是重新开始 还是继续训练
    ...
    iNet2 = torch.load('./models/iNet2_' + sys.argv[5] + '.pkl', map_location='cuda:'+sys.argv[1]).cuda()
...
optimizer2 = torch.optim.Adam([{'params':iNet2.parameters()}], lr=float(sys.argv[3]))
...
for i in range(int(sys.argv[4])):
    ...
    for j in range(16): # 每16批 当作一个训练单元 统计这16批数据的表现
        ...
        qEncData1 = quantize(encData1)
        decData1 = decNet1(qEncData1)
        encData2 = encNet1(decData1)
        qEncData2 = quantize(encData2)
        decData2 = decNet1(qEncData2)
