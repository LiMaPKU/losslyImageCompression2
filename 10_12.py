...
optimizer = torch.optim.Adam([{'params':encNet1.parameters()},{'params':decNet1.parameters()},{'params':encNet2.parameters()},{'params':decNet2.parameters()}], lr=float(sys.argv[3]))
...
for i in range(int(sys.argv[4])):
    ...
    for j in range(16): # 每16批 当作一个训练单元 统计这16批数据的表现
        ...
        currentMSEL = MSELoss(trainData, decData2)
        currentMS_SSIM = pytorch_msssim.ms_ssim(trainData, decData2, data_range=255, size_average=True)
        ...
        print('[%.3f'%currentMSEL.item(), '%.3f]'%currentMS_SSIM.item(), end='')
        sys.stdout.flush()
