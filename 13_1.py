...
for i in range(int(sys.argv[4])):
    ...
    for j in range(16): # 每16批 当作一个训练单元 统计这16批数据的表现
        ...
        if(i==0):
            sparseData = torch.ones_like(encData)*0.01

        currentKLL = KLLOSS(encData/255, sparseData)


        loss = -currentMS_SSIM + currentKLL
        ...
        loss.backward()
        optimizer.step()
        print(qEncData.max().item(),qEncData.min().item(),'[%.3f'%currentMS_SSIM.item(),'%.3f]'%currentKLL.item(),end='')
        sys.stdout.flush()
    ...
    print('minLoss=','%.3f'%minLoss.item(),'MSEL=','%.3f'%minLossMSEL.item(),'MS_SSIM=','%.3f'%minLossMS_SSIM.item(),'KLL=','%.3f'%minLossKLL.item())
