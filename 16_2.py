...
class EncodeNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = F.leaky_relu(self.conv_channels_up_1(x)) + F.leaky_relu(self.conv_channels_up_3(x)) + F.leaky_relu(self.conv_channels_up_5(x)) + F.leaky_relu(self.conv_channels_up_7(x))
...
class DecodeNet(nn.Module):
    ...
    def forward(self, x5):
        ...
        x = F.leaky_relu(self.tconv_channels_down_1(x1)) + F.leaky_relu(self.tconv_channels_down_3(x1)) + F.leaky_relu(self.tconv_channels_down_5(x1)) + F.leaky_relu(self.tconv_channels_down_7(x1))
...
for i in range(int(sys.argv[4])):
    ...
    for j in range(16): # 每16批 当作一个训练单元 统计这16批数据的表现
        ...
        if(defMaxLossOfTrainData==0):
            ...
            if(loss.item()>maxLossOfTrainData):
