...
if(sys.argv[2]=='0'): # 设置是重新开始 还是继续训练
    ...
    encNet3 = EncodeNet().cuda()
    decNet3 = DecodeNet().cuda()
    ...
    encNet3 = torch.load('./models/encNet3_' + sys.argv[5] + '.pkl', map_location='cuda:' + sys.argv[1]).cuda()
    decNet3 = torch.load('./models/decNet3_' + sys.argv[5] + '.pkl', map_location='cuda:' + sys.argv[1]).cuda()
...
optimizer3 = torch.optim.Adam([{'params':encNet3.parameters()},{'params':decNet3.parameters()}], lr=float(sys.argv[3]))
...
for i in range(int(sys.argv[4])):
    ...
    for j in range(16): # 每16批 当作一个训练单元 统计这16批数据的表现
        ...
        loss1.backward()
        optimizer1.step()
        # 编解码器1 输入trainData 输出decData1 逼近trainData
        ...
        loss2.backward()
        optimizer2.step()
        # 编解码器2 输入decData1 输出decData2 逼近trainData - decData1
        ...
        qEncData2 = quantize(encData2)
        decData2 = decNet2(qEncData2)
        encData3 = encNet3(decData1 + decData2)
        qEncData3 = quantize(encData3)
        decData3 = decNet3(qEncData3)
        loss3 = MSELoss(trainData - decData1 - decData2, decData3)
        loss3.backward()
        optimizer3.step()
        # 编解码器2 输入decData1 + decData2 输出decData3 逼近trainData - decData1 - decData2

        decData = decData1 + decData2 + decData3
        ...
        print(' [%.3f'%loss1.item(), '%.3f'%loss2.item(), '%.3f'%loss3.item(), '%.3f'%currentMSEL.item(), '%.3f] '%currentMS_SSIM.item(), end='')
        sys.stdout.flush()
    ...
    if (i == 0):
        ...
        if (minLoss > maxLossOfTrainData):  # 保存最小loss对应的模型
            ...
            torch.save(encNet3, './models/encNet3_' + sys.argv[5] + '.pkl')
            torch.save(decNet3, './models/decNet3_' + sys.argv[5] + '.pkl')
