...
class randomBMPDataset(Dataset):
    def __init__(self, bmpPath):
        self.imgNum = os.listdir(bmpPath).__len__()
        self.imgData = numpy.empty([self.imgNum, 1, 256, 256])

        for i in range(self.imgNum):
            img = Image.open(bmpPath + str(i) + '.bmp').convert('L')
            self.imgData[i] = numpy.asarray(img).astype(float).reshape([1, 256, 256])

        self.imgData = torch.from_numpy(self.imgData).float()


    def __getitem__(self, index):
        return self.imgData[index]
...
if(sys.argv[2]=='0'): # 设置是重新开始 还是继续训练
    ...
    encNet = torch.load('./models/encNet_' + sys.argv[5] + '.pkl')
    decNet = torch.load('./models/decNet_' + sys.argv[5] + '.pkl')
...
randomBMPLoader = DataLoader(dataset=randomBMPDataset('./256bmp'),batchSize=batchSize,shuffle=True)
...
for i in range(int(sys.argv[4])):
    ...
    for trainData in randomBMPLoader:
